...
def test_schema_provider_publishes_three_cost_resources() -> None:
    ...
    assert bundle.extensions["usageDependency"] == "meridian-plugin-usage>=2.0.2,<3"
...
def test_installed_distribution_and_entry_points_are_exact() -> None:
    assert version("meridian-plugin-cost") == "2.0.1"
    assert version("meridian-plugin-usage") in SpecifierSet(">=2.0.2,<3")
